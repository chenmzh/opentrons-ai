"""Jingle Bells：主歌 + 副歌 — precomposed, bounded phrase motion.

Run on the host: python3 protocols/ot2_jingle_bells.py
...
Source melody: https://www.mfiles.co.uk/scores/jingle-bells-main.pdf
...
metadata = {
    "protocolName": "OT-2 jingle-bells: precomposed full arrangement",
...
CHORUS = (
    (("E2", 1), ("E2", 1), ("E2", 2)),
    (("E2", 1), ("E2", 1), ("E2", 2)),
    (("E2", 1), ("G2", 1), ("C2", 1.5), ("D2", 0.5)),
    (("E2", 4),),
    (("F2", 1), ("F2", 1), ("F2", 1.5), ("F2", 0.5)),
    (("F2", 1), ("E2", 1), ("E2", 1), ("E2", 0.5), ("E2", 0.5)),
    (("E2", 1), ("D2", 1), ("D2", 1), ("E2", 1)),
    (("D2", 2), ("G2", 2)),
    (("E2", 1), ("E2", 1), ("E2", 2)),
    (("E2", 1), ("E2", 1), ("E2", 2)),
...
def score_phrases():
    bars = VERSE + CHORUS
    if any(sum(beats for _, beats in bar) != 4 for bar in bars):
        raise ValueError("Each Jingle Bells bar must have four beats")
    return tuple(bars[i] + bars[i + 1] for i in range(0, len(bars), 2))
...
def run(protocol):
    ...
    protocol.comment("JINGLE_BELLS_BEGIN")
    ...
    protocol.comment("JINGLE_BELLS_END")
    final = position()
...
if __name__ == "__main__":
    ...
    raise SystemExit(main(Path(__file__).resolve(), arrangement["notes"],
                          title="Jingle Bells：主歌 + 副歌", slug="jingle-bells",
