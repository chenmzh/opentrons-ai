"""Song of Storms：完整旋律两遍 — precomposed, bounded phrase motion.

Run on the host: python3 protocols/ot2_song_of_storms_full.py
...
Source melody: https://www.vgleadsheets.com/assets/sheets/C/The%20Legend%20of%20Zelda%20-%20Ocarina%20of%20Time%20-%20Song%20of%20Storms.pdf
...
metadata = {
    "protocolName": "OT-2 song-of-storms-full: precomposed full arrangement",
...
MARGIN_MM = 5.0
HZ_PER_MM_S = 4.98375
TEMPO_QUARTER_BPM = 132.0
MAX_SPEED_MM_S = 36.0
MIDI = {"D2": 38, "E2": 40, "F2": 41, "G2": 43, "A2": 45,
        "C3": 48, "D3": 50, "E3": 52, "F3": 53}
BARS = (
    (("D2", 0.5), ("F2", 0.5), ("D3", 2)),
    (("D2", 0.5), ("F2", 0.5), ("D3", 2)),
    (("E3", 1.5), ("F3", 0.5), ("E3", 0.5), ("F3", 0.5)),
    (("E3", 0.5), ("C3", 0.5), ("A2", 2)),
    (("A2", 1), ("D2", 1), ("F2", 0.5), ("G2", 0.5)),
    (("A2", 3),),
    (("A2", 1), ("D2", 1), ("F2", 0.5), ("G2", 0.5)),
    (("E2", 3),),
    (("D2", 0.5), ("F2", 0.5), ("D3", 2)),
...
def score_phrases():
    if any(sum(beats for _, beats in bar) != 3 for bar in BARS):
        raise ValueError("Each Song of Storms bar must have three beats")
    one_pass = tuple(sum(BARS[i:i + 2], ()) for i in range(0, len(BARS), 2))
...
def run(protocol):
    ...
    protocol.comment("SONG_OF_STORMS_FULL_BEGIN")
    ...
    protocol.comment("SONG_OF_STORMS_FULL_END")
    final = position()
...
if __name__ == "__main__":
    ...
    raise SystemExit(main(Path(__file__).resolve(), arrangement["notes"],
                          title="Song of Storms：完整旋律两遍", slug="song-of-storms-full",
